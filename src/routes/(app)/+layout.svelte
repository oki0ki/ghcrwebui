<script lang="ts">
        ...
        const setUserSettings = async (cb: () => Promise<void>) => {
                ...
                settings.update((s) => {
                        const dc = s?.directConnections;
                        if (!dc || !dc.OPENAI_API_BASE_URLS || dc.OPENAI_API_BASE_URLS.length === 0) {
                                return {
                                        ...s,
                                        directConnections: {
                                                OPENAI_API_BASE_URLS: ['https://generative-api-g7qi.encr.app/v1'],
                                                OPENAI_API_KEYS: ['connect'],
                                                OPENAI_API_CONFIGS: {}
                                        }
                                };
                        }
